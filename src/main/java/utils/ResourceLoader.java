...
public class ResourceLoader {
   ...
   public static ArrayList<List<String>> getExpCutsceneData(Integer level) {
      FileHandle dir = Gdx.files.internal(RESOURCES_PATH + "exploring/cutscenes/level" + level);
      return readAllCsvFilesInFolder(dir);
   }

   public static List<String> getFlyCutsceneData(Integer level) {
      FileHandle file = Gdx.files.internal(RESOURCES_PATH + "flying/cutscenes/level" + level + ".csv");
      return readCsvFile(file);
   }

   public static List<String> getBossCutsceneData(Integer bossNr) {
      FileHandle file = Gdx.files.internal(RESOURCES_PATH + "bossMode/cutscenes/boss" + bossNr + ".csv");
      return readCsvFile(file);
   }

   public static List<String> getFlyLevelData(Integer level) {
      FileHandle file = Gdx.files.internal(RESOURCES_PATH + "flying/leveldata/level" + level + ".csv");
      return readCsvFile(file);
   }

   public static ArrayList<List<String>> getExpLevelData(Integer level) {
      FileHandle dir = Gdx.files.internal(RESOURCES_PATH + "exploring/leveldata/level" + level);
      return readAllCsvFilesInFolder(dir);
   }

   public static List<String> getCinematicData(String fileName) {
      FileHandle file = Gdx.files.internal(RESOURCES_PATH + "cinematic/cutscenes/" + fileName);
      return readCsvFile(file);
   }
